...
private sealed class Robot(val name: String, val costs: Costs, val inServiceCount:Int, val inventory: Int){
    class Ore( oreCost: Int, inServiceCount: Int, inventory: Int): Robot("Ore",
        Costs(oreCost,0,0) , inServiceCount, inventory)
    class Clay( oreCost: Int, inServiceCount: Int, inventory: Int): Robot("Clay",
        Costs(oreCost,0, 0) , inServiceCount, inventory)
    class Obsidian( oreCost: Int,  clayCost: Int, inServiceCount: Int, inventory: Int): Robot("Obsidian",
        Costs(oreCost,clayCost, 0) , inServiceCount, inventory)
    class Geode( oreCost: Int,  obsidianCost: Int, inServiceCount: Int, inventory: Int): Robot("Geode",
        Costs(oreCost,0, obsidianCost) , inServiceCount, inventory)
...
fun main() {
    fun part1(input: List<String>): Int {
        ...
        return input.map(Blueprint3.Companion::of).sumOf { it ->
...
//            }
            ...
            while (queue.any()){
                ...
                queue.addLast(
                    FactoryState2(t-1, ore - it.costs.oreCost + oreRob, clay + clayRob, obs + obsRob, geo + geoRob, oreRob + 1, clayRob, obsRob,
                    geoRob)
                )
                if (ore >= it.costs.clayCost)
                queue.addLast(
                    FactoryState2(t-1, ore - it.costs.clayCost + oreRob, clay + clayRob, obs + obsRob, geo + geoRob, oreRob, clayRob + 1, obsRob,
                    geoRob)
                )
                if (ore >= it.costs.obsOreCost && clay >= it.costs.obsClayCost)
                queue.addLast(
                    FactoryState2(t-1, ore - it.costs.obsOreCost + oreRob, clay - it.costs.obsClayCost + clayRob, obs + obsRob, geo + geoRob, oreRob,
                    clayRob, obsRob + 1, geoRob)
                )
                if (ore >= it.costs.geoOreCost && obs >= it.costs.geoObsCost)
                queue.addLast(
                    FactoryState2(t-1, ore - it.costs.geoOreCost + oreRob, clay + clayRob, obs - it.costs.geoObsCost + obsRob, geo + geoRob, oreRob,
